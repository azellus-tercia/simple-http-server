...
class HttpParserTest {
    ...
    void parseHttpRequestWithBody() {
        ...
        try {
            request = httpParser.parseHttpRequest(
                    generateValidGETTestCaseWithBody()
            );
        } catch (HttpParsingException e) {
            fail(e);
        }
        ...
        assertEquals("HTTP/1.1", request.getOriginalHttpVersion());
        assertEquals(HttpVersion.HTTP_1_1, request.getBestCompatibleHttpVersion());
        assertEquals(request.getMessageBody().length, Integer.valueOf(request.getHeader("Content-Length")));
    }

    private InputStream generateValidGETTestCaseWithBody() {
